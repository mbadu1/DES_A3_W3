...
class TestDataCleaning(unittest.TestCase):
    ...
    def test_read_fn_columns_exist(self):
        ...
        for col in expected_columns:
            self.assertIn(
                col, self.df.columns, f"Column '{col}' missing from DataFrame"
            )

    def test_read_fn_data_types(self):
        self.assertTrue(
            pd.api.types.is_object_dtype(self.df["Date"]),
            "Date should be object type initially",
        )
        self.assertTrue(
            pd.api.types.is_numeric_dtype(self.df["SPX"]),
            "SPX should be numeric"
        )
        self.assertTrue(
            pd.api.types.is_numeric_dtype(self.df["GLD"]),
            "GLD should be numeric"
        )
        self.assertTrue(
            pd.api.types.is_numeric_dtype(self.df["SLV"]),
            "SLV should be numeric"
        )
    ...
    def test_process_date_fn_datetime_conversion(self):
        ...
        self.assertIsInstance(
            df_processed.index, pd.DatetimeIndex,
            "Index should be DatetimeIndex"
        )
        self.assertNotIn(
            "Date",
            df_processed.columns,
            "Date column should be removed after setting as index",
        )
    ...
    def test_no_all_null_columns(self):
        for column in self.df.columns:
            self.assertFalse(
                self.df[column].isnull().all(),
                f"Column '{column}' should not be entirely null",
            )
    ...
    def test_numeric_columns_have_valid_ranges(self):
        ...
        self.assertTrue(
            (df_processed["SPX"] > 0).all(),
            "SPX values should be positive"
        )
        self.assertTrue(
            (df_processed["SPX"] < 10000).all(),
            "SPX values should be reasonable (< 10000)",
        )

        # GLD should be positive
        self.assertTrue(
            (df_processed["GLD"] > 0).all(), "GLD values should be positive"
        )

        # SLV should be positive
        self.assertTrue(
            (df_processed["SLV"] > 0).all(), "SLV values should be positive"
        )
    ...
    def test_date_range_validity(self):
        ...
        self.assertGreaterEqual(
            min_date.year, 2015, "Minimum date should be 2015 or later"
        )
        self.assertLessEqual(
            max_date.year, 2025, "Maximum date should be 2025 or earlier"
        )
    ...
    def test_correlation_calculation(self):
        ...
        self.assertIsInstance(
            corr_matrix, pd.DataFrame, "Correlation should return DataFrame"
        )
        self.assertEqual(
            corr_matrix.shape[0],
            corr_matrix.shape[1],
    ...
    def test_data_continuity(self):
        ...
        # Check that no single day change is more
        # than 50% (reasonable for most cases)
...
class TestDataAnalysis(unittest.TestCase):
    ...
    that could be extracted from main script"""

    def setUp(self):
        # Create sample data for analysis tests
        dates = pd.date_range("2020-01-01", periods=100, freq="D")
        self.sample_df = pd.DataFrame(
            {"SPX": range(3000, 3100), "GLD":
                range(150, 250), "SLV":
                range(20, 120)},
            index=dates,
        )
